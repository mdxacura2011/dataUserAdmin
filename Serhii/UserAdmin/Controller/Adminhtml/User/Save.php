...
namespace Serhii\UserAdmin\Controller\Adminhtml\User;
...
/**
 * Preference save admin user data
 */
class Save extends \Magento\User\Controller\Adminhtml\User\Save
{
    ...
    public function execute()
    {
        ...
        $data = $this->optimizationProfilePhotoData($data);
     ...
     * @return mixed
     */
    private function optimizationProfilePhotoData($data)
    {
        if (is_array($data['profile_photo'])) {
            !empty($data['profile_photo']['delete']) ? $data['profile_photo'] = "" : $data['profile_photo'] = $data['profile_photo']['value'];
        }
